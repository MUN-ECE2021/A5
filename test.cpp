...
TEST(THE_IRON_THRONE, SPOILERS_AHEAD) {
	...
	ASSERT_EQ(GoT.getUser(5).name_, "Cerci Lannister");
	...
	ASSERT_EQ(GoT.getUser(5).degree(), 2);
	ASSERT_EQ(GoT.getUser(0).degree(), 4);
	ASSERT_EQ(GoT.getUser(1).degree(), 4);
	...
	ASSERT_EQ(JohnSnow.distance(RobStark), 1);
	ASSERT_EQ(JohnSnow.distance(CerciLannister), SIZE_MAX);
	...
	ASSERT_EQ(JohnSnow.distance(CerciLannister), 3);
	...
	ASSERT_EQ(JohnSnow.distance(TheMountain), 4);
	ASSERT_EQ(JohnSnow.distance(LittleFinger), SIZE_MAX);
...
TEST(RANDOM_1K_TESTS, FILL_WITH_VALUES)
{
	...
	//	ASSERT_EQ(itTest.userCount_, 1000);
}
...
TEST(RANDOM_50K_TESTS, ITERATE_THROUGH_NETWORK)
{
	...
	val = 0;
	for (auto i = itTest.begin(); i != itTest.end(); i++, val++) {
		//..	
	}
	ASSERT_EQ(val, 50000);
}
...
TEST(RANDOM_1K_TESTS, SEQUENTIAL_DISTANCE)
{
	SocialNetwork itTest;
	int val;
	int testSize = 1000;
	...
	for (int i = 0; i < val; i += 1) {
		for (int j = 0; j < 1; j++) {
			if (i < val - 1) {
				itTest.addFriendship(i, i + 1);
			}
		}
	}

	ASSERT_EQ(itTest.getUser(2).degree(), 2);
	...
	for (int i = 0; i < 20; i++) {
		...
		b = rand() % val;
		//	B = itTest.getUser(b);
		//	cout << "Dist from " << a << " to " << b << " = " << A.distance(B) << endl;
		if (a > b) {
			ASSERT_EQ(itTest.getUser(a).distance(itTest.getUser(b)), a - b);
			ASSERT_EQ(itTest.getUser(b).distance(itTest.getUser(a)), a - b);
		}
		else {
			ASSERT_EQ(itTest.getUser(a).distance(itTest.getUser(b)), b - a);
			ASSERT_EQ(itTest.getUser(b).distance(itTest.getUser(a)), b - a);
		}
	}
}

TEST(RANDOM_1K_TESTS, NON_SEQUENTIAL_DISTANCE)
{
	SocialNetwork itTest;
	srand(0);
	int val;
	int testSize = 1000;
	for (int i = 0; i < testSize; i++) {
		itTest.addUser(to_string(rand() % 1000));
	}
	//	ASSERT_EQ(itTest.userCount_, 1000);
	...
	ASSERT_EQ(itTest.getUser(20).degree(), 12);

	ASSERT_EQ(itTest.getUser(1).distance(itTest.getUser(2)), 4);
	ASSERT_EQ(itTest.getUser(2).distance(itTest.getUser(1)), 4);

	ASSERT_EQ(itTest.getUser(20).distance(itTest.getUser(202)), 4);
	ASSERT_EQ(itTest.getUser(202).distance(itTest.getUser(20)), 4);

	ASSERT_EQ(itTest.getUser(777).distance(itTest.getUser(666)), 2);
	ASSERT_EQ(itTest.getUser(666).distance(itTest.getUser(777)), 2);

	itTest.addUser("noFriends");
	const User * noFriends = &itTest.getUser(1000);
	ASSERT_EQ(itTest.getUser(1).distance(*noFriends), 4294967295);
	ASSERT_EQ(noFriends->distance(itTest.getUser(1)), 4294967295);
	ASSERT_EQ(itTest.getUser(1000).distance(itTest.getUser(1)), 4294967295);

}

TEST(RANDOM_1K_TESTS, FRIEND_ITERATOR_TEST)
{
	SocialNetwork itTest;
	int val;
	int testSize = 1000;
	...
	for (int i = 0; i < testSize; i++) {
		itTest.addFriendship(0, i);
	}

	ASSERT_EQ(itTest.getUser(0).degree(), 999);

	val = 0;
	for (auto i = itTest.getUser(0).begin(); i != itTest.getUser(0).end(); i++, val++) {
		// ...
	}
...
TEST(RANDOM_1K_TESTS, FIND_TEST)
{
	SocialNetwork itTest;
	int val;
	int testSize = 1000;
	for (int i = 0; i < testSize; i++) {
		//val = rand() % 1000;
		//itTest.addUser(to_string(val));
		itTest.addUser(to_string(i));
	}

	val = 0;
	for (auto i = itTest.find("888"); i != itTest.end(); i++, val++) {
		// ...
	}
	...
	val = 0;
	for (auto i = itTest.find("1"); i != itTest.end(); i++, val++) {
		// ...
	}
	...
	val = 0;
	for (auto i = itTest.find("0"); i != itTest.end(); i++, val++) {
		// ...
	}
